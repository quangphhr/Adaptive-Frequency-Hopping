...
void setup() {
  ...
  if (DATA_ACQ_MODE) {
    PACKAGE_NUM = 10000*long(MAX_CHANNEL-BASE_CHANNEL)+1;
    hash = 1;
  }
    ...
    if (present_time >= start_hopping_time) {
      if (flag_send_slot) {
        ...
        if (message_count >= PACKAGE_NUM && DATA_ACQ_MODE && (PACKAGE_TIMES >1)) {
          if (data_acq_times < PACKAGE_TIMES) {message_count = 0;}
